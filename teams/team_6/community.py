...
from teams.team_0.distributions_easy import (
    ability_distribution as default_ability_distribution,
)
from teams.team_0.distributions_easy import (
    task_difficulty_distribution as default_task_difficulty_distribution,
)
...
class Community:
    def __init__(
        self, num_abilities: int, num_players: int, player_distribution, members
    ):
...
class CommunityActions:
    ...
    def generate_tasks(community: Community, task_distribution):
        ...
        for _ in range(num_tasks):
            ...
            try:
                task_distri = task_distribution(
                    community.num_abilities,
                    seed_task_difficulty,
                    global_task_generation_id,
                    global_random,
                )
            except Exception as e:
                print(e)
                print(
                    f"Unable to use given task distribution function. Using the default task distribution function."
                )
                task_distri = default_task_difficulty_distribution(
                    community.num_abilities,
                    seed_task_difficulty,
                    global_task_generation_id,
                    global_random,
            ...
            assert (
                min(task_distri) >= 0 and max(task_distri) <= 10
            ), "Tasks difficulties should be in the range [0, 10]"
            assert (
                len(task_distri) == community.num_abilities
            ), f"Length of task distri ({len(task_distri.difficulties)}) does not match num_abilities ({community.num_abilities})"
    ...
    def simulate_turn(community: Community, task_distribution, available_players):
        ...
        for task in partnerships:
            ...
            player1 = next(
                member for member in community.members if member.id == player1ID
            )
            player2 = next(
                member for member in community.members if member.id == player2ID
            )
        ...
        individual_tasks, available_players = CommunityActions.assign_individual_tasks(
            community, available_players
        )
    ...
    def form_partnerships(community: Community, available_players):
        ...
        for player in available_players:
            ...
            # except Exception:
                ...
                num_calls_prefI += 1
                print(
                    f"Error getting partnership preferences for player {player.id} from group {player.group}. Assuming no preferences."
                )
            if pref is not None:
                bids[player.id] = pref
        ...
        for player_id in available_players_id:
            if player_id in bids:
                for task_id, partner_id in bids[player_id]:
                    assert task_id < len(
                        community.tasks
                    ), "Index should be within range while assigning partnership tasks"
                    task = community.tasks[task_id]
                    assert (
                        task in community.tasks
                    ), "The preferred task is not a community task, select valid tasks"
        ...
        sorted_tasks = sorted(
            all_partnerships.keys(), key=lambda t: sum(t), reverse=True
        )
    ...
    def assign_individual_tasks(community: Community, available_players):
        ...
        for player in available_players:
            ...
            try:
                ...
                print(e)
                print(
                    f"Error getting individual preferences for player {player.id} from group {player.group}. Assuming no preferences."
                )
                num_calls_prefII += 1
            # if tasksVolunteered:
            ...
            if tasksVolunteered is not None:
                for tasks_id in tasksVolunteered:
                    assert tasks_id < len(
                        community.tasks
                    ), "Index should be within range while assigning individual tasks"
                    tasks = community.tasks[tasks_id]
                    assert (
                        tasks in community.tasks
                    ), "The preferred task is not a community task, select valid tasks"
...
def getPairPreferencesPhaseI(player, community):
    ...
    func = import_class_from_file(
        f"teams/team_{group}", "preferences", "phaseIpreferences"
    )
...
def run_simulation(
    ...
    for group, count in player_distribution.items():
        for _ in range(count):
            try:
                distri = ability_distribution(
                    num_abilities, seed_ability, j, global_random
                )
            except Exception as e:
                print(e)
                print(
                    "Unable to use the given ability distribution function. Using the default ability distribution function."
                )
                distri = default_ability_distribution(
                    num_abilities, seed_ability, j, global_random
                )
            assert (
                min(distri) >= 0 and max(distri) <= 10
            ), "Ability distribution should be between [0, 10]"
            assert (
                len(distri) == num_abilities
            ), f"Length of distri ({len(distri)}) does not match num_abilities ({num_abilities})"
...
def create_gui(community, run_simulation_func, task_difficulty_distribution, num_turns):
    ...
    total_members_label = ttk.Label(
        overview_frame, text=f"Total Members: {len(community.members)}"
    )
    total_members_label.pack()
    active_members_label = ttk.Label(
        overview_frame,
    ...
    for i in range(num_abilities):
        ...
        ax.bar(
            x_positions + i * bar_width / num_abilities,
            abilities[:, i],
    ...
    ax.set_xticks(
        x_positions + bar_width * (num_abilities + 1) / 2 / (2 * num_abilities)
    )
    ...
    zoom_out_button = ttk.Button(
        ability_dist_frame, text="Zoom Out", command=auto_zoom_out
    )
    ...
    completed_tasks_label = ttk.Label(
        metrics_frame, text=f"Completed Tasks: {community.completed_tasks}"
    )
    completed_tasks_label.pack()

    # Task Completion Details
    task_completion_frame = ttk.LabelFrame(
        dashboard_frame, text="Task Completion Details"
    )
    task_completion_frame.grid(
        row=2, column=0, columnspan=2, padx=10, pady=10, sticky="nsew"
    )

    task_completion_tree = ttk.Treeview(
        task_completion_frame, columns=("Type", "Members", "Task"), show="headings"
    )
    ...
    member_tree = ttk.Treeview(
        member_frame,
        columns=("ID", "Group", "Energy", "Abilities", "Status"),
        show="headings",
    )
    ...
    def update_gui():
        ...
        for member in community.members:
            ...
            formatted_abilities = ", ".join(
                f"{ability:2d}" for ability in member.abilities
            )
            member_tree.insert(
                "",
                "end",
        ...
        # Update metrics
        completed_tasks_label.config(
            text=f"Completed Tasks: {community.completed_tasks}"
        )

        # Update community overview
        total_members_label.config(text=f"Total Members: {len(community.members)}")
        active_members_label.config(
            text=f"Active Members: {sum(1 for m in community.members if not m.incapacitated)}"
        )
    ...
    def run_turn():
        ...
        if current_turn < num_turns:
            ...
            available_players = {p for p in community.members if not p.incapacitated}
            final_partnerships, individual_tasks = run_simulation_func(
                community, task_difficulty_distribution, available_players
            )

            # Update task completion details with partnerships
            for task, partners in final_partnerships.items():
                task_completion_tree.insert(
                    "",
                    "end",
                    values=(
                        "Partnership",
                        f"{partners[0]}, {partners[1]}",
                        f"{task} (Sum: {sum(task)})",
                    ),
                )
            ...
            for task, player in individual_tasks:
                task_completion_tree.insert(
                    "",
                    "end",
                    values=("Individual", player.id, f"{task} (Sum: {sum(task)})"),
                )
        ...
        if current_turn >= num_turns:
            messagebox.showinfo(
                "Simulation Complete", f"Simulation completed after {num_turns} turns."
            )
    ...
    run_all_button = ttk.Button(
        task_frame, text="Run Entire Simulation", command=run_entire_simulation
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Total number of members in the community",
    )
    ...
    parser.add_argument(
        "--seed", type=int, default=42, help="Seed for entire simulation"
    )
    parser.add_argument(
        "--seed_ability",
        type=int,
        default=42,
        help="Seed for ability distribution randomness",
    )
    parser.add_argument(
        "--seed_task_difficulty",
        type=int,
        default=42,
        help="Seed for task difficulty randomness",
    )
    parser.add_argument(
        "--group_task_distribution",
        type=int,
        default=0,
        choices=range(0, 11),
    ...
    parser.add_argument(
        "--abilities_distribution_difficulty",
        type=str,
        default="easy",
        choices=["easy", "medium", "hard"],
        help="Easy (e), medium (m) or hard difficulty",
    ...
    parser.add_argument(
        "--gui", action="store_true", help="Run the simulation with GUI"
    )
    for i in range(1, 11):
        parser.add_argument(
            f"--g{i}", type=int, default=0, help=f"Number of players in group {i}"
        )
    ...
    try:
        ability_distribution = import_class_from_file(
            f"teams/team_{ability_team}",
            file_name + f"_{args.abilities_distribution_difficulty}",
            function_name_ability,
        )
    except Exception as e:
        print(e)
        ability_distribution = import_class_from_file(
            f"teams/team_0", file_name + "_easy", function_name_ability
        )
        print(
            f"Import from team {ability_team} failed. Using default ability distribution"
        )

    if args.log:
        ability_distribution = create_logged_function(
            ability_distribution, f"./log-results/team_{ability_team}_ability"
        )

    try:
        task_difficulty_distribution = import_class_from_file(
            f"teams/team_{task_difficulty_distribution_team}",
            file_name + f"_{args.task_distribution_difficulty}",
            function_name_task_difficulty,
        )

    except Exception as e:
        print(e)
        task_difficulty_distribution = import_class_from_file(
            f"teams/team_0", file_name + "_easy", function_name_task_difficulty
        )
        print(
            f"Import from team {task_difficulty_distribution_team} failed. Using default task difficulty distribution"
        )
    if args.log:
        task_difficulty_distribution = create_logged_function(
            task_difficulty_distribution,
    ...
    if args.gui:
        ...
        total_tasks_completed = run_simulation(
            num_abilities,
            num_members,
            player_distribution,
            num_turns,
        ...
        print(
            f"Average number of tasks per turn: {round(total_tasks_completed / num_turns, 2)}"
        )
    print(
        f"Average time taken for giving phase I preferences: {round(time_prefI/num_calls_prefI, 2)} seconds"
    )
    print(
        f"Average time taken for giving phase II preferences: {round(time_prefII / num_calls_prefII, 2)} seconds"
    )
